...
auto Planner::GetFuncCallFromFactory(const std::string &func_name, std::vector<AbstractExpressionRef> args)
  ...
  if (func_name == "lower" && args.size() == 1) {
    // StringExpression se(args[0], StringExpressionType::Lower);
    result = std::make_shared<StringExpression>(args[0], StringExpressionType::Lower);
  } else if (func_name == "upper" && args.size() == 1) {
    result = std::make_shared<StringExpression>(args[0], StringExpressionType::Upper);
  } else {
